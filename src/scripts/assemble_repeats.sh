...
cd $REFERENCE"_k_mer_words_locations"
...
if [ ! -f $REFERENCE"_hash_file"* ]; then
	$SMALT_BIN index -k 17 -s 17 $REFERENCE"_hash_file" $REFERENCE
fi

$RENAME_FASTQ_BIN -name "contig" -len 100 -num 2 "k"$K_SIZE"-2.fasta" "k"$K_SIZE"-2.fastq"

$SMALT_BIN map -m 20 -f ssaha -n 4 -O -d 0 $REFERENCE"_hash_file" "k"$K_SIZE"-2.fastq"

SHRED_SIZE=500
$SSAHA_SHRED_BIN -rlength $SHRED_SIZE $REFERENCE $REFERENCE"-shred-"$SHRED_SIZE"bp.fasta"
$SMALT_BIN -m 20 -f ssaha -n 4 -O -d 0 $REFERENCE"_hash_file" $REFERENCE"-shred-"$SHRED_SIZE"bp.fasta"
